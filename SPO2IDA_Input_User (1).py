...
# Get the values from the user for Yielding
yielding_values = input("Enter Yielding values (format: Vb, 𝛥): ").split(',')
yielding = [float(value) for value in yielding_values]

# Get the values from the user for Hardening
hardening_values = input("Enter Hardening values (format: Vb, 𝛥): ").split(',')
hardening = [float(value) for value in hardening_values]

# Get the values from the user for Softening
softening_values = input("Enter Softening values (format: Vb, 𝛥): ").split(',')
softening = [float(value) for value in softening_values]

# Get the values from the user for Residual
residual_values = input("Enter Residual values (format: Vb, 𝛥): ").split(',')
residual = [float(value) for value in residual_values]

# Get the values from the user for Degradation
degradation_values = input("Enter Degradation values (format: Vb, 𝛥): ").split(',')
degradation = [float(value) for value in degradation_values]

# Get the number of stories from the user
story = int(input("Enter the number of stories: "))

modal = []
for i in range(story):
    m, T = input("Enter modal data for story {} (format: m T): ".format(i + 1)).split()
    modal.append((m, T))
